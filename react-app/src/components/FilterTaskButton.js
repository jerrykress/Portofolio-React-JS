...
const FilterTaskButton = (props) => {
    ...
    return (
        <div className="relative" onMouseOver={showDropdown} onMouseLeave={hideDropdown}>
            <button className={`inline-flex items-center h-10 px-4 py-4 mb-3 mr-3 text-${(usedFilter==='None' ? 'gray' : 'indigo')}-400 border border-indigo-100 transition-colors duration-150 bg-white rounded-lg focus:outline-none hover:border-indigo-500 hover:text-indigo-700`}>
                <span>{`Filter by: ${usedFilter}`}</span>
